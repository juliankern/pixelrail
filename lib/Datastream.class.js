...
const defaultPort = '/dev/tty.usbserial-1430';
const defaultConfig = {
    serialport: {
        baudRate: 2000000,
        // databits: 8,
...
module.exports = class Datastream extends EventEmitter {
    constructor(port, config = {}) {
        ...
        this.port = port;
        this.serialport = new SerialPort(port || defaultPort, this.config.serialport, function (err) {
            if (err) {
    ...
    onSerialData(data) {
        console.log('onSerialData', this.port, data.toString());
        if (data.toString().split(/\r?\n/).includes(SIGNAL_READY)) {
            console.log('got ready', this.port);
            this._sendNextChunk();
    ...
    _sendNextChunk() {
        ...
        if (!this.dataToBeSent.length) {
            ...
            starttime = Date.now();
            // console.log('this.nextDataToBeSent', this.nextDataToBeSent);
        ...
        if (process.argv[2] === 'test') {
            console.log('sending testdata...', this.port, this.dataToBeSent.length, nextChunk);
        }
