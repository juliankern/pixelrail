...
module.exports = class rgbLight extends HomeKit {
    ...
    get debouncedUpdate() {
        return debounce(this.updatePixel, 100);
    }

    updatePixel(zero) {
        // console.log('updatepixel called', this.oldValues, this.values);
        if (JSON.stringify(this.oldValues) === JSON.stringify(this.values)) return;

        let oldRgb = hsvToRgb(this.oldValues.h, this.oldValues.s, this.oldValues.l);
        let rgb = hsvToRgb(this.values.h, this.values.s, this.values.l);

        if (!this.values.on) rgb = { r: 0, g: 0, b: 0 };
        if (!this.oldValues.on) oldRgb = { r: 0, g: 0, b: 0 };
        ...
        // console.log('set oldvalues with:', this.values);
